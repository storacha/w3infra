...
test(
  ...
  withCauseLog(async (t) => {
    ...
    const { invocationConfig, connection } = await getClientConfig(
      new URL(getApiEndpoint())
    )
      ...
      Array.from({ length: 4 }).map(async () => {
        ...
        await client.uploadFile(file, {
          onShardStored: (meta) => {
            ...
            if (!meta.piece)
              throw new Error('missing piece link in upload meta')
            uploadFiles.push({
              content,
              piece: meta.piece,
            })
            console.log(
              `file written with root: ${content}, shard: ${meta.cid}, piece: ${meta.piece}`
            )
          },
        })
    ...
    await Promise.race(
      uploads.map(async (testUpload) => {
        ...
        const roundaboutUrl = new URL(
          testUpload.piece.toString(),
          roundaboutEndpoint
        )
        await pWaitFor(
          async () => {
        ...
        console.log(
          'roundabout redirected from piece to blob',
          roundaboutUrl.toString()
        )

        // Invoke `filecoin/offer`
        console.log(
          `invoke 'filecoin/offer' for piece ${testUpload.piece.toString()} (${
            testUpload.content
          })`
        )
        const filecoinOfferRes = await Storefront.filecoinOffer(
          invocationConfig,
          testUpload.content,
          testUpload.piece,
          { connection }
        ...
        if (filecoinOfferRes.out.error) {
          throw new Error(
            `failed ${FilecoinCapabilities.offer.can} invocation`,
            { cause: filecoinOfferRes.out.error }
          )
        }
        ...
        console.log(
          'filecoin/offer fork (filecoin/submit)',
          filecoinSubmitReceiptCid
        )
        console.log(
          'filecoin/offer join (filecoin/accept)',
          filecoinAcceptReceiptCid
        )
        if (!filecoinAcceptReceiptCid) {
          return t.fail(
            'filecoin/offer receipt has no effect for filecoin/accept'
          )
        }
        ...
        const workflowLocation =
          workflowWithReceiptResponse.headers.get('location')
        if (!workflowLocation) {
          return t.fail(
            `no workflow with receipt for task: ${filecoinOfferInvCid}`
          )
        }

        const workflowWithReceiptResponseAfterRedirect = await fetch(
          workflowLocation
        )
        // Get receipt from Message Archive
        const agentMessageBytes = new Uint8Array(
          await workflowWithReceiptResponseAfterRedirect.arrayBuffer()
        )
        ...
        console.log(
          `wait for filecoin/submit receipt ${filecoinSubmitReceiptCid.toString()} ...`
        )
        ...
        const pieceOfferReceiptCid =
          receiptFilecoinSubmitRes.ok?.fx.join?.link()
        if (!pieceOfferReceiptCid) {
          throw new Error(
            'filecoin/submit receipt has no effect for piece/offer'
          )
        }
