...
test('upload-api /metrics', async t => {
   ...
   */
  t.is((body.match(/w3up_bytes/g) || []).length, 5)
  /**
   ...
   */
  t.is((body.match(/w3up_invocations_total/g) || []).length, 7)
})
 ...
 * 11. Verify metrics
 */
skipIfWritesDisabled(test)('w3infra store/upload integration flow', withCauseLog(async t => {
  const stage = getStage()
