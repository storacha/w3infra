...
import { createDynamodDb, createS3, createBucket } from './helpers/resources.js'
...
test('handles a batch of single invocation with aggregate/offer', async (t) => {
  const { tableName, workflowBucketName, invocationTableName } =
  ...
  const invocationStore = createInvocationStore(REGION, invocationTableName, {
    s3Address: t.context.s3Opts,
    dynamoAddress: t.context.dynamoOpts,
  })
  ...
  // Validate metrics
  const aggregateOfferTotal = await getItemFromTable(
  ...
  const aggregateOfferPiecesTotal = await getItemFromTable(
    t.context.dynamoClient,
    tableName,
    {
  ...
  t.is(
    aggregateOfferPiecesTotal?.name,
    METRICS_NAMES.AGGREGATE_OFFER_PIECES_TOTAL
  )
  ...
  const aggregateOfferPiecesSizeTotal = await getItemFromTable(
    t.context.dynamoClient,
    tableName,
    {
  ...
  t.is(
    aggregateOfferPiecesSizeTotal?.name,
    METRICS_NAMES.AGGREGATE_OFFER_PIECES_SIZE_TOTAL
  )
...
test('handles a batch of single invocation with multiple aggregate/offer attributes', async (t) => {
  ...
  t.is(
    aggregateOfferPiecesTotal?.value,
    aggregateOffers.reduce((acc, ao) => {
 ...
 * @param {{ workflowBucketName: string, invocationTableName: string, s3: import('@aws-sdk/client-s3').S3Client, dynamo: import('@aws-sdk/client-dynamodb').DynamoDBClient }} ctx
 */
async function prepareUcanStream(workflows, ctx) {
  const storageService = await Signer.generate()

  return Promise.all(
    workflows.map(async (aggregateOffers) => {
      const invocationsToExecute = await Promise.all(
        aggregateOffers.map(async (agg) => {
      ...
      const request = await encodeAgentMessage({
        invocations: invocationsToExecute.map((ie) => ie.invocation),
      })
      ...
      await Promise.all(
        invocationsToExecute.map(async (ie) => {
          const putObjectCmd = new PutItemCommand({
            TableName: ctx.invocationTableName,
            Item: {
              taskkind: {
                S: `${ie.invocation.cid.toString()}.in`,
              },
              identifier: {
          ...
          await ctx.dynamo.send(putObjectCmd)
        })
      )

      // Create UCAN Stream Invocation
      const streamData = fromString(
        JSON.stringify({
          carCid: agentMessageCarCid,
          value: {
            att: invocationsToExecute.map((ie) =>
              DealerCapabilities.aggregateOffer.create({
                with: storageService.did(),
                nb: ie.params,
              })
            ),
...
async function prepareResources(dynamoClient, s3Client) {
  const [tableName, workflowBucketName, invocationTableName] =
    await Promise.all([
      ...
      createDynamoTable(dynamoClient, agentIndexTableProps),
    ])
