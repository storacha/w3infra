...
export async function followFilecoinReceiptChain() {
  const { ENV, PIECE_CID, PRIVATE_KEY } = getEnv()
  ...
  const filecoinSubmitReceiptGet = await receiptStore.get(
    filecoinSubmitInvocation.link()
  )
  ...
  if (!filecoinSubmitReceiptGet.ok.fx.join)
    throw new Error('receipt without effect')
  console.log('piece/offer', filecoinSubmitReceiptGet.ok.fx.join)

  // Get `piece/offer` receipt
  const pieceOfferReceiptGet = await receiptStore.get(
    filecoinSubmitReceiptGet.ok.fx.join.link()
  )
  ...
  const filecoinAcceptReceiptGet = await receiptStore.get(
    filecoinAcceptInvocation.link()
  )
