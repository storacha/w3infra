...
export const extract = (archive) => {
  let rts, blks
  try {
    console.error('decoding CAR')
    const { roots, blocks } = CAR.decode(archive)
    rts = roots
    blks = blocks
  } catch (err) {
    console.error('failed to decode CAR', err)
    throw err
  }
  ...
  return view({ root: rts[0], blocks: blks })
}
